...
if (newsCount.count === 0) {
    ...
    const insertNews = db.prepare(`
        INSERT INTO news (title, content, category, author, is_featured, is_highlight)
        VALUES (?, ?, ?, ?, ?, ?)
    `);
...
app.post('/api/news', (req, res) => {
    try {
        ...
        const stmt = db.prepare(`
            INSERT INTO news (title, content, category, image, author, is_featured, is_highlight)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
...
app.put('/api/news/:id', (req, res) => {
    try {
        ...
        const stmt = db.prepare(`
            UPDATE news 
            SET title = ?, content = ?, category = ?, image = ?, author = ?, is_featured = ?, is_highlight = ?
            WHERE id = ?
        `);
